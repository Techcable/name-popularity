...
def insert_name(
    ...
    *,
    male_count: int,
    female_count: int,
    male_rank: Optional[int],
    female_rank: Optional[int],
):
...
def load_year_file(conn: sqlite3.Connection, year: int, location: Path):
    ...
    with conn:
        conn.execute(
            "INSERT INTO years (year, total_males, total_females) VALUES (?, ?, ?);",
            (year, total_males, total_females),
        )
    ...
    def rank_names(known_names: set[str], gender: str) -> dict[str, int]:
        return {
            ...
            for index, name in enumerate(
                sorted(
                    known_names,
                    key=lambda name: name_counts[name][gender],
                    reverse=True,
                )
            )
        }
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        ...
        print(
            "Usage: ./convert_database.py <database file> <names directory>",
            file=sys.stderr,
        )
        sys.exit(2)
